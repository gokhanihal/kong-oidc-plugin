...
local basic_serializer = require "kong.plugins.http-log-atez.basic"
local BatchQueue = require "kong.tools.batch_queue"
local cjson_safe = require "cjson.safe"
local url = require "socket.url"
local http = require "resty.http"
--local zzlib = require "kong.plugins.http-log-atez.zzlib"
...
function HttpLogHandler:body_filter(conf)    
  ...
  if conf.log_body and conf.max_body_size > 0 then
    ...
    if (headers["content-encoding"]) ~="gzip" then
      ngx.log(ngx.DEBUG, "================================================================================")
      ngx.log(ngx.DEBUG, "RESPONSE BODY HTTP LOG...")
      ngx.log(ngx.DEBUG, "================================================================================")
      ...
      if string.len(ngx.ctx.response_body)<conf.max_body_size then
        ...
        --ngx.ctx.response_body = res_body      
      end  
    end      
  else
    ngx.ctx.request_body = ""
...
function HttpLogHandler:log(conf)
  ...
  if not q then
    ...
    local process = function(entries)
      ...
      payload = "{\"Entry\":\"" .. ngx_encode_base64(payload) .. "\" }"
