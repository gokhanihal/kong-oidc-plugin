...
local OidcTokenHandler = BasePlugin:extend()


OidcTokenHandler.VERSION  = "1.0.0"
OidcTokenHandler.PRIORITY = 10
...
function OidcTokenHandler:new()
  OidcTokenHandler.super.new(self, "oidc-token-atez")
end

function OidcTokenHandler:access(config)
  OidcTokenHandler.super.access(self)
  ...
  if authStr and (string.find(authStr,'basic ') or string.find(authStr,'Basic ')) then
    local basicToken = string.sub(authStr, 7);
    local tokenStr = ngx.decode_base64(basicToken);
    
    local parts = {}
    for pattern in string.gmatch(tokenStr, "[^:]+") do
      table.insert(parts, pattern)
    end
    
    -- local count = 0
    -- for _ in pairs(table) do count = count + 1 end
    if #parts ~= 2 then return kong.response.exit(ngx.HTTP_UNAUTHORIZED, "Invalid password") end
